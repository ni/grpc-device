...
frame1 = nixnet_types.FrameBuffer(
   enet = nixnet_types.EnetFrame(
      type = 0, #nxEnetFrameType_Data
      device_timestamp = 0,
      network_timestamp = 0,
      flags = 0,
      frame_data = bytes([
...
convert_signal_response = client.ConvertSignalsToFramesSinglePoint(
   nixnet_types.ConvertSignalsToFramesSinglePointRequest(
      ...
      frame_type = nixnet_types.FrameType.FRAME_TYPE_ENET
   )
)
...
for f in convert_signal_response.frame_buffer:
   print(f.enet.flags)
   print(f.enet.frame_data)
   print(len(f.enet.frame_data))
