  ...
  ::grpc::Status NiRFmxWLANService::AnalyzeNWaveformsIQ(::grpc::ServerContext* context, const AnalyzeNWaveformsIQRequest* request, AnalyzeNWaveformsIQResponse* response)
  {
    ...
    try {
      ...
      if (total_length != request->iq_size()) {
        return ::grpc::Status(::grpc::INVALID_ARGUMENT, "The total size of iq did not match the exected size from iq_lengths");
      }

      auto iq_lengths = const_cast<int32*>(reinterpret_cast<const int32*>(request->iq_lengths().data()));
      ...
      if (array_size_size_calculation.match_state == MatchState::MISMATCH) {
        return ::grpc::Status(::grpc::INVALID_ARGUMENT, "The sizes of linked repeated fields [x0, dx, iqLengths] do not match");
      }
      auto array_size = array_size_size_calculation.size;

      int32 reset = request->reset();
      auto status = library_->AnalyzeNWaveformsIQ(instrument, selector_string, result_name, x0, dx, iq.data(), iq_lengths, array_size, reset);
      response->set_status(status);
      return ::grpc::Status::OK;
    }
    catch (nidevice_grpc::LibraryLoadException& ex) {
