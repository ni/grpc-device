...
namespace nidaqmx_grpc {
    ...
    try {
      ...
      if (status == 0) {
        for (int i = 0; i < stateVector.size(); ++i) {
          response->add_states(static_cast<DigitalPullUpPullDownStates>(stateVector[i]));
        }
      }
      return ::grpc::Status::OK;
    }
  ...
  ::grpc::Status NiDAQmxService::SetDigitalPowerUpStates(::grpc::ServerContext* context, const SetDigitalPowerUpStatesRequest* request, SetDigitalPowerUpStatesResponse* response)
  {
    ...
    try {
      auto device_name = request->device_name().c_str();
      auto get_channelNames_if = [](const google::protobuf::RepeatedPtrField<DigitalPowerUpState>& vector, int n) -> const char* {
            if (vector.size() > n) {
                  return vector[n].channelnames().c_str();
            }
            return nullptr;
      };
      auto get_state_if = [](const google::protobuf::RepeatedPtrField<DigitalPowerUpState>& vector, int n) -> int32 {
            if (vector.size() > n) {
                  return vector[n].state();
            }
            return 0;
