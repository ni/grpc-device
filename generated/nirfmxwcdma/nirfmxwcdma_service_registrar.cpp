...
struct LibraryAndService {
  LibraryAndService(
    ...
    const NiRFmxWCDMAFeatureToggles& feature_toggles)
      : library(std::make_shared<NiRFmxWCDMALibrary>()),
      service(std::make_shared<NiRFmxWCDMAService>(
        library,
        resource_repository,
        vi_session_resource_repository,
        feature_toggles)) {
  }
  std::shared_ptr<NiRFmxWCDMALibrary> library;
  std::shared_ptr<NiRFmxWCDMAService> service;
};
}
