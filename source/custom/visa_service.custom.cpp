...
static ViSession GetResourceManagerSession(VisaService::LibrarySharedPtr library)
{
...
static std::tuple<ViObject, bool> GetObjectValueWithType(const SessionOrEventData& data, VisaService::ResourceRepositorySharedPtr session_repository)
{
  if (data.has_vi()) {
    auto vi_grpc_session = data.vi();
    return { session_repository->access_session(vi_grpc_session.name()), true };
  }
  else if (data.has_event_handle()) {
    return { data.event_handle(), false };
  }
  else {
    throw nidevice_grpc::NonDriverException(::grpc::INVALID_ARGUMENT, "The value for object_handle was not specified or out of range");
  }
}
    ...
    try {
      ...
      switch (GetAttributeType(attributeID))
      {
        case AttributeValueData::kValueString: {
            ...
            status = library_->GetAttribute(vi, attributeID, attrValue);
            response->mutable_attribute_value()->mutable_value_string()->assign(attrValue);
            break;
        }

        case AttributeValueData::kValueU64: {
            ViUInt64 val64;
            status = library_->GetAttribute(vi, attributeID, &val64);
            response->mutable_attribute_value()->set_value_u64(val64);
            break;
        }

        case AttributeValueData::kValueU32: {
            ViUInt32 val32;
            status = library_->GetAttribute(vi, attributeID, &val32);
            response->mutable_attribute_value()->set_value_u32(val32);
            break;
        }

        case AttributeValueData::kValueU16: {
            ViUInt16 val16;
            status = library_->GetAttribute(vi, attributeID, &val16);
            ...
            response->mutable_attribute_value()->set_value_u16(val16);
            break;
        }

        case AttributeValueData::kValueBool: {
            ViBoolean valBool;
            status = library_->GetAttribute(vi, attributeID, &valBool);
            response->mutable_attribute_value()->set_value_bool(valBool == VI_TRUE);
            break;
        }

        case AttributeValueData::kValueU8: {
            ViUInt8 val8;
            status = library_->GetAttribute(vi, attributeID, &val8);
            response->mutable_attribute_value()->set_value_u8(val8);
            break;
        }
        ...
        case AttributeValueData::kValueI32: {
            ViInt32 val32;
            status = library_->GetAttribute(vi, attributeID, &val32);
            response->mutable_attribute_value()->set_value_i32(val32);
            break;
        }
        ...
        case AttributeValueData::kValueI16: {
            ViInt16 val16;
            status = library_->GetAttribute(vi, attributeID, &val16);
            response->mutable_attribute_value()->set_value_i16(val16);
            break;
        }

        default:
            return ConvertApiErrorStatusForViSession(context, VI_ERROR_NSUP_ATTR, vi);
      }
