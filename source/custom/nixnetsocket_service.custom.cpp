...
::grpc::Status NiXnetSocketService::ConvertApiErrorStatusForNxSOCKET(::grpc::ServerContext* context, google::protobuf::int32 status, nxSOCKET socket)
{
...
::grpc::Status NiXnetSocketService::ConvertApiErrorStatusForNxIpStackRef_t(::grpc::ServerContext* context, google::protobuf::int32 status, nxIpStackRef_t socket)
{
  // This implementation assumes this method is always called on the same thread where the error occurred.
  std::string description(nidevice_grpc::kMaxGrpcErrorDescriptionSize, '\0');
  library_->GetLastErrorStr(&description[0], nidevice_grpc::kMaxGrpcErrorDescriptionSize);
