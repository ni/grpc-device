...
TEST_F(NiRFmxWLANDriverApiTests, DSSSModAccFromExample_FetchData_DataLooksReasonable)
{
  ...
  DSSSModAccFetchEVMResponse dsss_mod_acc_fetch_evm_response;
  DSSSModAccFetchPPDUInformationResponse dsss_mod_acc_fetch_ppdu_information_response;
  DSSSModAccFetchIQImpairmentsResponse dsss_mod_acc_fetch_iq_impairments_response;
  DSSSModAccFetchEVMPerChipMeanTraceResponse dsss_mod_acc_fetch_evm_per_chip_mean_trace_response;
  DSSSModAccFetchConstellationTraceResponse dsss_mod_acc_fetch_constellation_trace_response;
  dsss_mod_acc_fetch_evm_response = client::dsss_mod_acc_fetch_evm(stub(), session, "", 10.0);
  dsss_mod_acc_fetch_ppdu_information_response = client::dsss_mod_acc_fetch_ppdu_information(stub(), session, "", 10.0);
  dsss_mod_acc_fetch_iq_impairments_response = client::dsss_mod_acc_fetch_iq_impairments(stub(), session, "", 10.0);
  dsss_mod_acc_fetch_evm_per_chip_mean_trace_response = client::dsss_mod_acc_fetch_evm_per_chip_mean_trace(stub(), session, "", 10.0);
  dsss_mod_acc_fetch_constellation_trace_response = client::dsss_mod_acc_fetch_constellation_trace(stub(), session, "", 10.0);
  ...
  EXPECT_EQ(0.0, dsss_mod_acc_fetch_evm_response.chip_clock_error_mean());
  EXPECT_EQ(0, dsss_mod_acc_fetch_evm_response.number_of_chips_used());
  EXPECT_SUCCESS(session, dsss_mod_acc_fetch_ppdu_information_response);
  EXPECT_EQ(0, dsss_mod_acc_fetch_ppdu_information_response.data_modulation_format());
  ...
  EXPECT_SUCCESS(session, dsss_mod_acc_fetch_evm_per_chip_mean_trace_response);
  EXPECT_EQ(0.0, dsss_mod_acc_fetch_evm_per_chip_mean_trace_response.x0());
  EXPECT_EQ(0.0, dsss_mod_acc_fetch_evm_per_chip_mean_trace_response.dx());
  EXPECT_EQ(999, dsss_mod_acc_fetch_evm_per_chip_mean_trace_response.evm_per_chip_mean_size());
  EXPECT_EQ(999, dsss_mod_acc_fetch_evm_per_chip_mean_trace_response.evm_per_chip_mean().size());
  EXPECT_EQ(0.0, dsss_mod_acc_fetch_evm_per_chip_mean_trace_response.evm_per_chip_mean(0));
  EXPECT_SUCCESS(session, dsss_mod_acc_fetch_constellation_trace_response);
  EXPECT_EQ(0.0, dsss_mod_acc_fetch_constellation_trace_response.constellation(0).real());
  EXPECT_EQ(0.0, dsss_mod_acc_fetch_constellation_trace_response.constellation(0).imaginary());
}

TEST_F(NiRFmxWLANDriverApiTests, DSSSPowerRampFromExample_FetchData_DataLooksReasonable)
{
  char* resourceName = "RFSA";
  auto instr_stub = create_stub<nirfmxinstr_grpc::NiRFmxInstr>();
  auto session = init_instr_session(instr_stub, resourceName);
  ...
  float64 x0 = 0;
  float64 dx = 0;
  PowerRampFetchMeasurementResponse power_ramp_fetch_measurement_response;
  PowerRampFetchRiseTraceResponse power_ramp_fetch_rise_trace_response;
  PowerRampFetchFallTraceResponse power_ramp_fetch_fall_trace_response;
  power_ramp_fetch_measurement_response = client::power_ramp_fetch_measurement(stub(), session, "", 10.0);
  power_ramp_fetch_rise_trace_response = client::power_ramp_fetch_rise_trace(stub(), session, "", 10.0);
  x0 = 0;
  dx = 0;
  power_ramp_fetch_fall_trace_response = client::power_ramp_fetch_fall_trace(stub(), session, "", 10.0);
  ...
  EXPECT_EQ(999, power_ramp_fetch_rise_trace_response.raw_waveform_size());
  EXPECT_EQ(999, power_ramp_fetch_rise_trace_response.raw_waveform().size());
...
TEST_F(NiRFmxWLANDriverApiTests, OFDMModAccFromExample_FetchData_DataLooksReasonable)
{
  ...
  OFDMModAccFetchNumberofSymbolsUsedResponse ofdm_mod_acc_fetch_numberof_symbols_used_response;
  ...
  OFDMModAccFetchPPDUTypeResponse ofdm_mod_acc_fetch_ppdu_type_response;
  OFDMModAccFetchMCSIndexResponse ofdm_mod_acc_fetch_mcs_index_response;
  OFDMModAccFetchGuardIntervalTypeResponse ofdm_mod_acc_fetch_guard_interval_type_response;
  OFDMModAccFetchLSIGParityCheckStatusResponse ofdm_mod_acc_fetch_lsig_parity_check_status_response;
  OFDMModAccFetchSIGCRCStatusResponse ofdm_mod_acc_fetch_sigcrc_status_response;
  OFDMModAccFetchSIGBCRCStatusResponse ofdm_mod_acc_fetch_sigbcrc_status_response;
  ...
  OFDMModAccFetchChainRMSEVMPerSubcarrierMeanTraceResponse ofdm_mod_acc_fetch_chain_rmsevm_per_subcarrier_mean_trace_response;
  ofdm_mod_acc_fetch_composite_rmsevm_response = client::ofdm_mod_acc_fetch_composite_rmsevm(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_numberof_symbols_used_response = client::ofdm_mod_acc_fetch_numberof_symbols_used(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_frequency_error_mean_response = client::ofdm_mod_acc_fetch_frequency_error_mean(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_symbol_clock_error_mean_response = client::ofdm_mod_acc_fetch_symbol_clock_error_mean(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_iq_impairments_response = client::ofdm_mod_acc_fetch_iq_impairments(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_ppdu_type_response = client::ofdm_mod_acc_fetch_ppdu_type(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_mcs_index_response = client::ofdm_mod_acc_fetch_mcs_index(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_guard_interval_type_response = client::ofdm_mod_acc_fetch_guard_interval_type(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_lsig_parity_check_status_response = client::ofdm_mod_acc_fetch_lsig_parity_check_status(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_sigcrc_status_response = client::ofdm_mod_acc_fetch_sigcrc_status(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_sigbcrc_status_response = client::ofdm_mod_acc_fetch_sigbcrc_status(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_pilot_constellation_trace_response = client::ofdm_mod_acc_fetch_pilot_constellation_trace(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_data_constellation_trace_response = client::ofdm_mod_acc_fetch_data_constellation_trace(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_chain_rmsevm_per_subcarrier_mean_trace_response = client::ofdm_mod_acc_fetch_chain_rmsevm_per_subcarrier_mean_trace(stub(), session, "", 10.0);
...
TEST_F(NiRFmxWLANDriverApiTests, OFDMModAccMIMOFromExample_FetchData_DataLooksReasonable)
{
  ...
  for (int i = 0; i < NUMBER_OF_FREQUENCY_SEGMENTS; ++i)
  {
    ...
    EXPECT_SUCCESS(session, client::cfg_frequency(stub(), session, segmentString, centerFrequencyArray[i]));
  }
  ...
  if (PPDUType == NIRFMXWLAN_INT32_OFDM_PPDU_TYPE_MU)
  {
    ...
    for (int i = 0; i < numberOfUsers; i++)
    {
      ...
      ofdm_mod_acc_fetch_mcs_index_response = client::ofdm_mod_acc_fetch_mcs_index(stub(), session, userString, 10.0);
      ofdm_mod_acc_fetch_number_of_space_time_streams_response = client::ofdm_mod_acc_fetch_number_of_space_time_streams(stub(), session, userString, 10.0);
      spaceTimeStreamOffsetArray[i] = get_attr_i32(session, userString, NIRFMXWLAN_ATTRIBUTE_OFDMMODACC_RESULTS_SPACE_TIME_STREAM_OFFSET);
  ...
  for (int i = 0; i < NUMBER_OF_FREQUENCY_SEGMENTS; i++)
  {
    auto segment_string_response = client::build_segment_string(stub(), "", i);
    EXPECT_SUCCESS(session, segment_string_response);
    ofdm_mod_acc_fetch_frequency_error_mean_response = client::ofdm_mod_acc_fetch_frequency_error_mean(stub(), session, segmentString, 10.0);
    ofdm_mod_acc_fetch_symbol_clock_error_mean_response = client::ofdm_mod_acc_fetch_symbol_clock_error_mean(stub(), session, segmentString, 10.0);
    for (int j = 0; j < numberOfStreamResults; j++)
    {
      auto stream_string_response = client::build_stream_string(stub(), segmentString, j);
      EXPECT_SUCCESS(session, stream_string_response);
      ofdm_mod_acc_fetch_stream_rmsevm_response = client::ofdm_mod_acc_fetch_stream_rmsevm(stub(), session, streamString, 10.0);
      ofdm_mod_acc_fetch_stream_rmsevm_per_subcarrier_mean_trace_response = client::ofdm_mod_acc_fetch_stream_rmsevm_per_subcarrier_mean_trace(stub(), session, streamString, 10.0);
      ofdm_mod_acc_fetch_pilot_constellation_trace_response = client::ofdm_mod_acc_fetch_pilot_constellation_trace(stub(), session, streamString, 10.0);
      ofdm_mod_acc_fetch_data_constellation_trace_response = client::ofdm_mod_acc_fetch_data_constellation_trace(stub(), session, streamString, 10.0);
    }
    for (int j = 0; j < NUMBER_OF_RECEIVE_CHAINS; j++)
    {
      auto chain_string_response = client::build_chain_string(stub(), segmentString, j);
      EXPECT_SUCCESS(session, chain_string_response);
      ofdm_mod_acc_fetch_cross_power_response = client::ofdm_mod_acc_fetch_cross_power(stub(), session, chainString, 10.0);
      ofdm_mod_acc_fetch_iq_impairments_response = client::ofdm_mod_acc_fetch_iq_impairments(stub(), session, chainString, 10.0);
    }
  }
...
TEST_F(NiRFmxWLANDriverApiTests, OFDMModAccTriggerBasedPPDUFromExample_FetchData_DataLooksReasonable)
{
  ...
  OFDMModAccFetchCompositeRMSEVMResponse ofdm_mod_acc_fetch_composite_rmsevm_response;
  OFDMModAccFetchUnusedToneErrorResponse ofdm_mod_acc_fetch_unused_tone_error_response;
  OFDMModAccFetchUnusedToneErrorMarginPerRUResponse ofdm_mod_acc_fetch_unused_tone_error_margin_per_ru_response;
  OFDMModAccFetchFrequencyErrorMeanResponse ofdm_mod_acc_fetch_frequency_error_mean_response;
  OFDMModAccFetchFrequencyErrorCCDF10PercentResponse ofdm_mod_acc_fetch_frequency_error_ccdf10_percent_response;
  OFDMModAccFetchSymbolClockErrorMeanResponse ofdm_mod_acc_fetch_symbol_clock_error_mean_response;
  OFDMModAccFetchPPDUTypeResponse ofdm_mod_acc_fetch_ppdu_type_response;
  OFDMModAccFetchPilotConstellationTraceResponse ofdm_mod_acc_fetch_pilot_constellation_trace_response;
  OFDMModAccFetchDataConstellationTraceResponse ofdm_mod_acc_fetch_data_constellation_trace_response;
  OFDMModAccFetchUnusedToneErrorMeanTraceResponse ofdm_mod_acc_fetch_unused_tone_error_mean_trace_response;
  ofdm_mod_acc_fetch_composite_rmsevm_response = client::ofdm_mod_acc_fetch_composite_rmsevm(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_unused_tone_error_response = client::ofdm_mod_acc_fetch_unused_tone_error(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_unused_tone_error_margin_per_ru_response = client::ofdm_mod_acc_fetch_unused_tone_error_margin_per_ru(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_frequency_error_mean_response = client::ofdm_mod_acc_fetch_frequency_error_mean(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_frequency_error_ccdf10_percent_response = client::ofdm_mod_acc_fetch_frequency_error_ccdf10_percent(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_symbol_clock_error_mean_response = client::ofdm_mod_acc_fetch_symbol_clock_error_mean(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_ppdu_type_response = client::ofdm_mod_acc_fetch_ppdu_type(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_pilot_constellation_trace_response = client::ofdm_mod_acc_fetch_pilot_constellation_trace(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_data_constellation_trace_response = client::ofdm_mod_acc_fetch_data_constellation_trace(stub(), session, "", 10.0);
  ofdm_mod_acc_fetch_unused_tone_error_mean_trace_response = client::ofdm_mod_acc_fetch_unused_tone_error_mean_trace(stub(), session, "", 10.0);
  ...
  EXPECT_EQ(999, ofdm_mod_acc_fetch_unused_tone_error_margin_per_ru_response.unused_tone_error_margin_per_ru_size());
  EXPECT_EQ(999, ofdm_mod_acc_fetch_unused_tone_error_margin_per_ru_response.unused_tone_error_margin_per_ru().size());
  EXPECT_EQ(0.0, ofdm_mod_acc_fetch_unused_tone_error_margin_per_ru_response.unused_tone_error_margin_per_ru(0));
  EXPECT_SUCCESS(session, ofdm_mod_acc_fetch_frequency_error_mean_response);
  EXPECT_EQ(0.0, ofdm_mod_acc_fetch_frequency_error_mean_response.frequency_error_mean());
  EXPECT_SUCCESS(session, ofdm_mod_acc_fetch_frequency_error_ccdf10_percent_response);
  EXPECT_EQ(0.0, ofdm_mod_acc_fetch_frequency_error_ccdf10_percent_response.frequency_error_ccdf10_percent());
  EXPECT_SUCCESS(session, ofdm_mod_acc_fetch_symbol_clock_error_mean_response);
  EXPECT_EQ(0.0, ofdm_mod_acc_fetch_symbol_clock_error_mean_response.symbol_clock_error_mean());
  EXPECT_SUCCESS(session, ofdm_mod_acc_fetch_ppdu_type_response);
  EXPECT_EQ(0, ofdm_mod_acc_fetch_ppdu_type_response.ppdu_type());
  ...
  EXPECT_EQ(999, ofdm_mod_acc_fetch_unused_tone_error_mean_trace_response.unused_tone_error_size());
  EXPECT_EQ(999, ofdm_mod_acc_fetch_unused_tone_error_mean_trace_response.unused_tone_error().size());
  EXPECT_EQ(0.0, ofdm_mod_acc_fetch_unused_tone_error_mean_trace_response.unused_tone_error(0));
  EXPECT_EQ(999, ofdm_mod_acc_fetch_unused_tone_error_mean_trace_response.unused_tone_error_mask_size());
  EXPECT_EQ(999, ofdm_mod_acc_fetch_unused_tone_error_mean_trace_response.unused_tone_error_mask().size());
  EXPECT_EQ(0.0, ofdm_mod_acc_fetch_unused_tone_error_mean_trace_response.unused_tone_error_mask(0));
}
...
TEST_F(NiRFmxWLANDriverApiTests, OFDMModAccWithEVMBasedAutoLevelFromExample_FetchData_DataLooksReasonable)
{
  char* resourceName = "RFSA";
  auto instr_stub = create_stub<nirfmxinstr_grpc::NiRFmxInstr>();
  auto session = init_instr_session(instr_stub, resourceName);
...
TEST_F(NiRFmxWLANDriverApiTests, SemMIMOFromExample_FetchData_DataLooksReasonable)
{
  ...
  for (int i = 0; i < NUMBER_OF_FREQUENCY_SEGMENTS; ++i)
  {
    ...
    for (int j = 0; j < NUMBER_OF_RECEIVE_CHAINS; ++j)
    {
      auto chain_string_response = client::build_chain_string(stub(), segmentString, j);
      EXPECT_SUCCESS(session, chain_string_response);
      sem_fetch_carrier_measurement_response = client::sem_fetch_carrier_measurement(stub(), session, chainString, 10.0);
      sem_fetch_lower_offset_margin_array_response = client::sem_fetch_lower_offset_margin_array(stub(), session, chainString, 10.0);
      sem_fetch_upper_offset_margin_array_response = client::sem_fetch_upper_offset_margin_array(stub(), session, chainString, 10.0);
      arraySize = sem_fetch_upper_offset_margin_array_response.measurement_status_size();
      sem_fetch_spectrum_response = client::sem_fetch_spectrum(stub(), session, chainString, 10.0);
    }
  }
...
TEST_F(NiRFmxWLANDriverApiTests, TXPMIMOFromExample_FetchData_DataLooksReasonable)
{
  ...
  for (int i = 0; i < NUMBER_OF_FREQUENCY_SEGMENTS; ++i)
  {
    ...
    for (int j = 0; j < NUMBER_OF_RECEIVE_CHAINS; ++j)
    {
      auto chain_string_response = client::build_chain_string(stub(), segmentString, j);
      EXPECT_SUCCESS(session, chain_string_response);
      txp_fetch_measurement_response = client::txp_fetch_measurement(stub(), session, chainString, 10.0);
      txp_fetch_power_trace_response = client::txp_fetch_power_trace(stub(), session, chainString, 10.0);
    }
  }
