    ...
    GetCalibrationInformationResponse* response)
{
  ...
  auto status = get_calibration_data(
      ...
      &calibration_internal_last_reading,
      &calibration_internal_last_temperature_c,
    ...
    double* calibration_external_last_temperature_c)
{
  auto get_calibration_data = [&](nidevice_grpc::SysCfgLibraryInterface* library, NISysCfgResourceHandle resource, bool* save_changes) {
    ...
    char syscfg_name[NISYSCFG_SIMPLE_STRING_LENGTH] = '\0';
    ...
    *calibration_external_last_reading = *reinterpret_cast<CVIAbsoluteTime*>(&syscfg_time);

    status = library->GetResourceProperty(resource, NISysCfgResourcePropertyRecommendedNextCalibrationTime, &syscfg_time);
    if (NISysCfg_Failed(status)) {
      return status;
    }
    *calibration_external_next_reading = *reinterpret_cast<CVIAbsoluteTime*>(&syscfg_time);

    return status;
  };
  return access_syscfg_resource_by_device_id_filter(context, device_id, get_calibration_data);
}
