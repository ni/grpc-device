        ...
        if mechanism in {'len', 'ivi-dance', 'ivi-dance-with-a-twist'}:
            ...
            if mechanism == 'len':
                if 'determine_size_from' not in size_param:
                    ...
                    size_param['linked_params_are_optional'] = True
                size_param['determine_size_from'].append(param['name'])
                is_optional = common_helpers.is_optional(param)
                if is_optional and not size_param['linked_params_are_optional']:
                    raise Exception("Code generator does not support linked params that aare a mix of optional and required", size_param)
